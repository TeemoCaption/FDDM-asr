...
def evaluate_train_cer(
    ...
    注意：這裡使用「直接 argmax 還原」的簡化推論，而非完整 diffusion 取樣流程。
    """
    encoder.eval()
    decoder.eval()
    s_proj.eval()
    ...
    # 以固定次數包 tqdm，避免某些 DataLoader 無法 len() 時出錯
    try:
        total = min(max_batches, len(train_loader))
    except Exception:
        total = max_batches

    iterator = _iter_with_progress(range(total), desc="Eval[train]", total=total)
    ...
    for _ in iterator:
        ...
        refs = [_ids_to_text_one(x0[i], tokenizer, pad_id, bos_id, eos_id) for i in range(x0.size(0))]
        ...
        if len(cer_list) >= max_batches:  # 保險上限
            break

    return (sum(cer_list) / len(cer_list)) if cer_list else 1.0
